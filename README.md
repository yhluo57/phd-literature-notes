...
- 下拉与新建：分组、状态、材料体系、器件结构、主题、标签和物理机制都支持从已有值选择，也支持新建。
- 批量编辑：按当前筛选范围批量修改状态、分组、材料体系、器件结构，或批量添加标签/物理机制。
- GitHub 同步：填入 fine-grained token 后，可以手动同步，也可以打开自动同步模式。

## 路线图

路线图页按研究分组展示。选择分组后，页面会把该分组下全部文献按年份排成时间线，并生成关键参数汇总和全量对照表；重点图谱文献会保留状态标识，但不会作为唯一筛选条件。

## 材料索引

材料索引页会按 `material_system` 聚合文献，适合快速查找 MnGa、MnAl、CoGa、Ta、Pt、MgO、CoMnFe 等材料线索。
